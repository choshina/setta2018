...
Peng Wu\inst{1,2} \textsuperscript{(\Letter)} \and
Yu Zhang\inst{1,2}}
%
...
Active testing approaches have been proposed for concurrency bug localization based on bug patterns. %Potential bug locations can then be ranked with the suspicious bug patterns gathered. 
...
%We claim that the novelty of this work lie in that it focuses on the localization of linearizability faults correctness criterion of linearizability, and thus tree search also focuses on such a direction. We apply some existing tree search techniques such as partial-order reduction but we emphasizes our novel approach of localizing linearizability faults.
...
\item Let $e_m(\mathit{Arg})^{\left\langle o,t\right\rangle}$ be a memory access event in $S_f$. The thread identifier $o$ of $e_m$ must occur in a pair of matching invocation and response events $c(\mathit{Arg})^{\left\langle o,t\right\rangle}, r(\mathit{Arg})^{\left\langle o,t\right\rangle}$ in $S_f$.
 % \item All the memory access events with operation identifier $o$ lie between the consistent matching invocation/response event pair with the operation identifier $o$, i.e.,
  ...
  \item Let $c(\mathit{Arg})^{\left\langle o,t\right\rangle}, r(\mathit{Arg})^{\left\langle o,t\right\rangle}$ be a pair of matching invocation and response events in $S_f$. 
...
Intuitively, a \textit{CDRS} contains all the HLDRs which are decisive to the linearizability of the trace. Note that although different \textit{CDRS}es in a program $\mathbb{P}$ may lead to different linearizability faults, what we focus on is just the linearizability of the trace and thus we treat all linearizability faults identical in the sense that they all lead the trace non-linearizable.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \State $\mathcal{S}_t = \mathcal{S}_t^{init}$ \Comment{$\mathcal{S}_t$ is initialized}
        \State $\mathit{enS} = \left\{ e | \forall \epsilon\in \mathsf{M} .(\mathtt{td}(\epsilon)=\mathtt{td}(e)\longrightarrow e\prec \epsilon)\right\}$ \Comment{Foremost events of each thread}
        \Function{BuildTree}{$\mathcal{S}_t,\mathit{enS}$}
            ...
            \For{$e(addr,v_n)^{\pair{o,t}} \leftarrow \mathit{enS}$}
                ...
                \State $\mathcal{S}_t \gets \mathit{\mathcal{S}_t[v_n/addr]}$ \Comment{Update state}
                \State $\mathit{enS} \gets \mathit{enS}\setminus \sset{e} \cup \sset{e'}$ \Comment{Update $\mathit{enS}$}
                \State \Call{BuildTree}{$\mathcal{S}_t,\mathit{enS}$} \Comment{Recursively build the tree}
            \EndFor
        \EndFunction
    \end{algorithmic}
\end{algorithm}
...
\small
    ...
    \begin{algorithmic}[1]
        ...
        \Function {LabelNode}{$N_d$}
            \If{$!\mathtt{Left}(N_d)  \& !\mathtt{Right}(N_d)$} \Comment{$N_d$ is a leaf}
                ...
                %\State \textbf{end if}
            \ElsIf{$\mathtt{Left}(N_d)  \& !\mathtt{Right}(N_d)$} \Comment{$N_d$ has only left child}
                \State \Return \Call{LabelNode}{$\mathtt{Left}(N_d)$}
            \ElsIf{$!\mathtt{Left}(N_d)  \& \mathtt{Right}(N_d)$} \Comment{$N_d$ has only right child}
                \State \Return \Call{LabelNode}{$\mathtt{Right}(N_d)$}
            \Else \Comment{$N_d$ has both children}
                \State $\mathit{Label_l} = $ \Call{LabelNode}{$\mathtt{Left}(N_d)$}
                \State $\mathit{Label_r} = $ \Call{LabelNode}{$\mathtt{Right}(N_d)$}
                \State \textbf{switch} {$\pair{\mathit{Label_l},\mathit{Label_r}}$} \Comment{The label depends on 2 children}
